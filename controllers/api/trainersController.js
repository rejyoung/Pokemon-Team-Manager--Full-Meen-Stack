...
async function createProfile(req, res) {
    try {
        ...
        res.status(200).json({
            redirectURL: `/trainer/profile?token=${userToken}`,
        });
    } catch (error) {
        let errorObj = {
        ...
        if (typeof pokemonList != "undefined") {
            if (pokemonList.length > 0) {
                pokemonList.forEach(async (pokemon) => {
                    let targetSpecies = await Pokemon.findOne({
                        _id: pokemon.SpeciesID,
                    });
                    let trainerIndex = targetSpecies.TeamsOn.indexOf(
                        targetTrainer._id
                    );
                    if (trainerIndex != -1) {
...
async function logIn(req, res) {
    try {
        ...
        const targetTrainer = await Trainer.findOne({
    ...
    try {
        ...
        let nextEvolutionArray = await Promise.all(
            speciesNextEv.map(async (species) => {
                let evSpecId = await Pokemon.findOne({
...
async function levelPokemon(req, res) {
    try {
        ...
        // add new moves
        let moves = targetSpecies.Moves.filter(
            (move) => move.Level <= newLevel
        );
        ...
        if (
            ...
            let minimumLev =
                targetSpecies.EvolutionTree.firstEvolution[0].MinLevel;
        ...
        if (
            ...
            let minimumLev =
                targetSpecies.EvolutionTree.finalEvolution[0].MinLevel;
            if (newLevel >= minimumLev) {
                targetPokemon.CanEvolve = true;
                evolvePossible = true;
            } else {
        ...
        console.log(targetPokemon.CanEvolve);
        targetTrainer.markModified(`PokemonCollection.${pokemonIndex}`);
...
async function evolvePokemon(req, res) {
    try {
        ...
        if (
            ...
            let minimumLev =
                newSpecies.EvolutionTree.finalEvolution[0].MinLevel;
...
async function addPokemonToCurrentTeam(req, res) {
    try {
        ...
        let targetTrainer = await Trainer.findById(trainerId);
        if (
            targetTrainer.PokemonCollection.filter((mon) => mon.CurrentTeam)
                .length < 6
        ) {
