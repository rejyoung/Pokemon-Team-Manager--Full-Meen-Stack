document.addEventListener("DOMContentLoaded", () => {
    ...
    const manageAccountWindow = document.querySelector(
    ...
    const xShift = window.innerWidth < 700 ? "100vw" : "40vw";
    const yShift =
        window.innerWidth < 700
            ? String((window.innerHeight - 70) * -1)
            : "-550px";
    manageProfileBtn.addEventListener("click", () => {
        console.log("click");
        gsap.timeline()
            .set(manageAccountFrame, { display: "block" })
            .set(manageAccountWindow, { x: xShift, display: "block" })
    ...
    closeManageBtn.addEventListener("click", () => {
        gsap.timeline()
            .to(manageAccountWindow, { duration: 0.3, x: xShift })
    ...
    changeNameBtn.addEventListener("click", () => {
        gsap.timeline()
            .set(changeNameForm, { display: "flex" })
            .to(manageOptions, { duration: 0.5, y: yShift })
            .to(changeNameForm, { duration: 0.5, y: yShift }, "-=.5")
    ...
    changePassBtn.addEventListener("click", () => {
        gsap.timeline()
            ...
            .to(manageOptions, { duration: 0.5, y: yShift })
            .to(changePassForm, { duration: 0.5, y: yShift }, "-=.5")
            .set(manageOptions, { display: "none" })
    ...
    function closeChangeName() {
        gsap.timeline()
            .set(changeNameForm, { y: yShift })
            .set(manageOptions, { display: "flex", yShift })
    ...
    function closeChangePassword() {
        gsap.timeline()
            .set(changePassForm, { y: yShift })
            .set(manageOptions, { display: "flex", y: yShift })
            .to(changePassForm, { duration: 0.5, y: "0" })
            .to(manageOptions, { duration: 0.5, y: 0 }, "-=.5")
    ...
    newNamePassInput.addEventListener("input", () => {
        if (
            newNamePassInput.value.length > 7 &&
            newNameInput.value.length > 0
        ) {
            canSubmitName = true;
    ...
    changeNameSave.addEventListener("click", () => {
        if (canSubmitName) {
            try {
                axios
                    .put(
                        `/api/trainers/oneTrainer/${trainerToken}/changeDisplayName`,
                        {
    ...
    changePassSave.addEventListener("click", () => {
        if (canSubmit) {
            try {
                axios
                    .put(
                        `/api/trainers/oneTrainer/${trainerToken}/changePassword`,
                        {
