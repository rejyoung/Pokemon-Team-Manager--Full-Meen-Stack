...
server {
    listen 443 ssl;
    server_name ptm.ryanyoung.codes;
...
aws ssm get-parameter --name "/PTM/ssl/certificate" --with-decryption --output text --query Parameter.Value > /etc/pki/tls/certs/server.crt
if [ $? -ne 0 ]; then
...
aws ssm get-parameter --name "/PTM/ssl/privatekey" --with-decryption --output text --query Parameter.Value > /etc/pki/tls/certs/server.key
if [ $? -ne 0 ]; then
